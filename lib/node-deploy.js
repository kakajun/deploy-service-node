...
(async () => {
  ...
  try {
    ...
    if (versionFlagIndex > -1 || versionLongFlagIndex > -1) {
      ...
      await runCommand('tar', ['-czf', deployConfig.LOCAL_TAR_FILE, deployConfig.REMOTE_DISTNAME]);
      ...
      await transferFileToRemote(
        ...
        path.join(deployConfig.REMOTE_DIR, path.basename(deployConfig.LOCAL_TAR_FILE))
      );
      console.log('\x1b[32m正在将远程脚本文件传输到服务器...\x1b[0m');
      await transferFileToRemote(
        ssh,
        remoteSh,
        path.join(deployConfig.REMOTE_DIR, 'remote-deploy.sh'),
      );
      const CURRENT_TIMESTAMP = new Date().toISOString().replace(/[-:.TZ]/g, '');
      console.log('\x1b[32m正在远程服务器上执行脚本...\x1b[0m');
      await executeRemoteScript(ssh,
        path.posix.join(deployConfig.REMOTE_DIR,'remote-deploy.sh'), [
